...
import { fetch, update, deleteItem, setRefetch, deleteItemsByIds } from '../../stores/messages/messagesSlice'
...
const TableSampleMessages = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  const { messages, loading, count, notify: messagesNotify, refetch } = useAppSelector((state) => state.messages)
  ...
  useEffect(() => {
    loadColumns(handleDeleteModalAction, `messages`).then((newCols) =>
      setColumns(newCols),
    );
  }, []);
