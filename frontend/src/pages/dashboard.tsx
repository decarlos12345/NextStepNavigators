...
const Dashboard = () => {
    ...
    const [options, setOptions] = React.useState(loadingMessage);
    const [questions, setQuestions] = React.useState(loadingMessage);
    const [quizzes, setQuizzes] = React.useState(loadingMessage);

    async function loadData() {
        const entities = ['users','courses','messages','notes','options','questions','quizzes',];
        const fns = [setUsers,setCourses,setMessages,setNotes,setOptions,setQuestions,setQuizzes,];
    ...
    <>
      ...
      <SectionMain>
        ...
        <div id="dashboard" className='grid grid-cols-1 gap-6 lg:grid-cols-3 mb-6'>
                    ...
                    <div className="flex justify-between align-center">
                        ...
                        <div>
                            <BaseIcon
                                ...
                                // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                                // @ts-ignore
                                path={'mdiNoteOutline' in icon ? icon['mdiNoteOutline' as keyof typeof icon] : icon.mdiTable || icon.mdiTable}
                            />
                        </div>
                    </div>
                </div>
            ...
            <Link href={'/options/options-list'}>
                <div
                    className={`${corners !== 'rounded-full'? corners : 'rounded-3xl'} dark:bg-dark-900 ${cardsStyle} dark:border-dark-700 p-6`}
                >
                    <div className="flex justify-between align-center">
                        ...
                        <div>
                            <BaseIcon
                                ...
                                // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                                // @ts-ignore
                                path={'mdiCheckboxMarkedCircleOutline' in icon ? icon['mdiCheckboxMarkedCircleOutline' as keyof typeof icon] : icon.mdiTable || icon.mdiTable}
                            />
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex justify-between align-center">
                        ...
                        <div>
                            <BaseIcon
                                ...
                                // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                                // @ts-ignore
                                path={'mdiClipboardQuestionOutline' in icon ? icon['mdiClipboardQuestionOutline' as keyof typeof icon] : icon.mdiTable || icon.mdiTable}
                            />
                        </div>
                    </div>
                </div>
